...
/**
 * Grid-style listing card — the "discovery" card used on the homepage's
 * featured section and the /browse grid view, for either vertical.
 * Modeled on Booking.com's property card (photo-first, name/location, price
 * bottom-right), adapted to only show data we actually have: no star
 * ratings or review counts (no reviews system exists), no fabricated
 * "deal" badges — the hero photo is a real owner-uploaded room photo when
 * one exists.
 */
export function ListingCard({ listing }: { listing: DirectoryListing }) {
  ...
  return (
    ...
    >
      ...
      <div className="p-3.5">
        <h3 className="truncate text-[15px] font-bold" style={{ color: MP.ink }}>{listing.name}</h3>
        {location && (
        ...
        <div className="mt-3 flex items-end justify-between">
          ...
          <span className="text-[16px] font-bold" style={{ color: MP.green }}>{formatGHS(listing.from_rate)}</span>
        </div>
      </div>
    </Link>
  )
