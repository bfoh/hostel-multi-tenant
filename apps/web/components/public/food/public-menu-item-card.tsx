...
export function PublicMenuItemCard({ id, name, description, price_pesewas, photo_url, is_sold_out, qty, onChange, color }: Props) {
          ...
          {is_sold_out
            ...
            : qty === 0
              ...
              : <div className="inline-flex items-center gap-2 rounded-full border border-slate-200 px-1 py-0.5">
                  <button type="button" onClick={() => update(qty - 1)} className="rounded-full p-1 hover:bg-slate-100">
                    <Minus className="h-3 w-3" />
                  </button>
                  <span className="min-w-[1.25rem] text-center text-xs font-semibold">{qty}</span>
                  <button type="button" onClick={() => update(qty + 1)} disabled={qty >= 10} className="rounded-full p-1 hover:bg-slate-100">
