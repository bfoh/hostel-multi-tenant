...
export function DeleteCategoryButton({ id, name }: { id: string; name: string }) {
  ...
  if (confirming) {
    return (
      <div className="flex shrink-0 flex-col items-start gap-1 sm:items-end">
        ...
        {error && (
          <p className="max-w-[240px] text-[11px] text-red-600 sm:text-right">{error}</p>
        )}
      </div>
    )
  }
  ...
  return (
    <button
      ...
      title={`Delete ${name}`}
      className="shrink-0 rounded-md border border-border p-1.5 text-text-tertiary hover:bg-red-50 hover:text-red-600 hover:border-red-200 transition-colors"
    >
