...
export function RoomPhotosUpload({ categoryId, initialUrls }: {
  categoryId:  string
  initialUrls: string[]
}) {
  ...
  const onPick: React.ChangeEventHandler<HTMLInputElement> = async (e) => {
    ...
    fd.set('file', file)
    const res = await fetch(`/api/room-categories/${categoryId}/photos`, { method: 'POST', body: fd })
    setBusy(false)
  ...
  async function removePhoto(url: string) {
    ...
    const res = await fetch(`/api/room-categories/${categoryId}/photos`, {
      method:  'DELETE',
  ...
  return (
    <div className="space-y-2">
      ...
      <p className="text-xs text-text-secondary">
        Shown on your public marketplace listing and booking page. Up to {MAX_PHOTOS} photos, 4MB each.
      </p>
