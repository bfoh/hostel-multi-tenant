...
import {
  ...
  coerceBookingRow,
  BOOKING_BULK_HEADERS,
  BOOKING_REQUIRED_HEADERS,
} from '@/lib/validation/booking-import'
...
function validateHeaders(rawRows: Record<string, unknown>[]): string | null {
  ...
  const missing = BOOKING_REQUIRED_HEADERS.filter((h) => !headers.includes(h))
  if (missing.length) {
    return `Missing required column${missing.length > 1 ? 's' : ''}: ${missing.join(', ')}.`
  }
  // Soft check: at least one of occupant_phone or occupant_student_id present
  if (!headers.includes('occupant_phone') && !headers.includes('occupant_student_id')) {
    return 'Need at least one of: occupant_phone or occupant_student_id.'
  }
  return null
}
...
function processRows(raw: Record<string, unknown>[]): ParsedRow[] {
  return raw.map((r, i) => {
    const coerced = coerceBookingRow(r)
    const parsed = bookingImportRowSchema.safeParse(coerced)
    const errors: string[] = []
    if (!parsed.success) {
...
export function BulkBookingImportWizard() {
  ...
  function downloadTemplate() {
    const headers = [...BOOKING_BULK_HEADERS]
    const example: Record<string, string> = {
      occupant_phone: '0551109602',
      occupant_student_id: '',
      room_number: 'A101',
    ...
    const enums = [
      { field: 'source', allowed: 'walk_in | phone | website | widget | voice_ai | referral' },
      { field: 'check_in_date / check_out_date', allowed: 'YYYY-MM-DD (or DD/MM/YYYY)' },
      { field: 'discount_amount', allowed: 'integer, minor currency unit' },
      { field: 'occupant lookup', allowed: 'Either occupant_phone OR occupant_student_id (must exist)' },
      { field: 'room lookup', allowed: 'room_number (must exist in this hostel)' },
    ]
    const wsEnum = XLSX.utils.json_to_sheet(enums)
    XLSX.utils.book_append_sheet(wb, wsEnum, 'Reference')

    XLSX.writeFile(wb, 'bookings-template.xlsx')
  }

  function downloadErrorReport() {
    if (!result) return
    const data = result.errors.map((e) => ({ row: e.row, error: e.message }))
    const wb = XLSX.utils.book_new()
    const ws = XLSX.utils.json_to_sheet(data, { header: ['row', 'error'] })
    XLSX.utils.book_append_sheet(wb, ws, 'Errors')
    XLSX.writeFile(wb, 'bookings-import-errors.xlsx')
  }
  ...
  async function submitImport() {
    ...
    setSubmitting(true)
    try {
      const res = await fetch('/api/bookings/bulk-import', {
        method: 'POST',
      ...
      <div className="space-y-4">
        <div className="rounded-xl border border-border bg-surface p-6">
          <div className="flex flex-col items-center justify-center gap-4 rounded-lg border-2 border-dashed border-border bg-surface-sunken py-12 px-4 text-center">
            ...
            {fileName && <p className="text-xs text-text-tertiary">{fileName}</p>}
          </div>
        ...
        <div className="flex items-center justify-between rounded-xl border border-border bg-surface p-4">
          <div>
            ...
            <p className="text-xs text-text-secondary">
              Download an Excel template with required columns and example row.
            </p>
          </div>
          <button
        ...
        <div className="rounded-xl border border-border bg-surface p-4 text-xs text-text-secondary">
          ...
          <p className="font-mono">room_number, check_in_date, check_out_date, source, AND (occupant_phone OR occupant_student_id)</p>
          <p className="mt-3 mb-2 font-medium text-text-primary">Allowed values</p>
          <ul className="space-y-0.5 font-mono">
            <li>source: walk_in | phone | website | widget | voice_ai | referral</li>
            <li>dates: YYYY-MM-DD (also accepts DD/MM/YYYY)</li>
          </ul>
          <p className="mt-3 text-text-tertiary">Occupants and rooms must exist before import. Bookings created as pending_payment.</p>
        </div>
    ...
    <div className="space-y-4">
          ...
          <table className="w-full text-sm">
            <thead className="sticky top-0 bg-surface-sunken">
              <tr>
                ...
                <th className="w-10 px-3 py-2 text-left text-xs font-medium text-text-tertiary"></th>
                <th className="px-3 py-2 text-left text-xs font-medium text-text-tertiary">Occupant</th>
                <th className="px-3 py-2 text-left text-xs font-medium text-text-tertiary">Room</th>
                <th className="px-3 py-2 text-left text-xs font-medium text-text-tertiary">Check-in</th>
                <th className="px-3 py-2 text-left text-xs font-medium text-text-tertiary">Check-out</th>
            ...
            <tbody className="divide-y divide-border">
              {rows.map((r) => {
                ...
                const occupant = String(r.data.occupant_phone ?? '') || String(r.data.occupant_student_id ?? '') || '—'
                return (
                  <tr key={r.index} className={ok ? '' : 'bg-danger-subtle/40'}>
                    ...
                    <td className="px-3 py-2 ref-number text-text-secondary text-xs">{occupant}</td>
                    <td className="px-3 py-2 text-text-primary">{String(r.data.room_number ?? '')}</td>
                    <td className="px-3 py-2 text-text-secondary text-xs">{String(r.data.check_in_date ?? '')}</td>
                    <td className="px-3 py-2 text-text-secondary text-xs">{String(r.data.check_out_date ?? '')}</td>
                    <td className="px-3 py-2 text-text-secondary capitalize">{String(r.data.source ?? '').replace('_', ' ')}</td>
                    <td className="px-3 py-2">
                      {ok ? (
      ...
      <div className="flex items-center justify-end gap-2">
        ...
        >
          ...
          {submitting ? 'Importing…' : `Import ${stats.selected} booking${stats.selected !== 1 ? 's' : ''}`}
        </button>
      </div>
    </div>
  )
