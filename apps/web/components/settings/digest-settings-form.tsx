...
export function DigestSettingsForm({ initial, primary, timezone }: Props) {
  ...
  async function sendTest() {
    setTesting(true); setError(null); setTestResult(null); setTestErrors([])
    try {
      ...
      const summary = `Sent · SMS ${data.sms_sent} · Email ${data.email_sent} · Push ${data.push_sent}`
      setTestResult(data.reason ? `${summary} (${data.reason})` : summary)
  ...
  return (
    <div className="space-y-6">
      ...
      <section className="rounded-xl border border-border bg-surface p-5 space-y-3">
        <div className="flex items-start justify-between gap-3">
          ...
          <button
            ...
            className="shrink-0 flex items-center gap-1 rounded-md border border-border bg-surface-raised px-2 py-1 text-[11px] font-medium hover:bg-surface-sunken transition-colors"
          >
      ...
      {testErrors.length > 0 && (
        <div className="rounded-md bg-warning-subtle px-3 py-2 text-xs text-warning-fg space-y-1">
          <p className="font-semibold">Test issues</p>
          <ul className="list-disc pl-5 space-y-0.5">
            {testErrors.map((m, i) => <li key={i}>{m}</li>)}
          </ul>
        </div>
      )}
