...
import { Home, CalendarCheck, ClipboardList, MessageCircle, MoreHorizontal } from 'lucide-react'
...
const TABS: TabDef[] = [
  { href: '/staff-mobile',          label: 'Home',     Icon: Home,          anim: 'pulse'  },
  { href: '/staff-mobile/bookings', label: 'Bookings',  Icon: CalendarCheck, anim: 'slide'  },
  { href: '/staff-mobile/tasks',    label: 'Tasks',     Icon: ClipboardList, anim: 'swing'  },
  { href: '/staff-mobile/messages', label: 'Messages',  Icon: MessageCircle, anim: 'shake'  },
  { href: '/staff-mobile/more',     label: 'More',      Icon: MoreHorizontal, anim: 'bounce' },
]

export function StaffMobileBottomNav({ color, userId }: { color: string; userId: string }) {
  ...
  useEffect(() => {
    ...
    const ch = sb.channel(`staff-mobile-unread:${userId}`)
      .on('postgres_changes',
    ...
    >
      <div className="mx-auto flex max-w-2xl px-1">
        {TABS.map(({ href, label, Icon, anim }) => {
          const active = pathname === href || (href !== '/staff-mobile' && pathname.startsWith(href))
          return (
            <Link
              key={href}
              href={href}
            ...
            >
              ...
              >
                ...
                />
                {href === '/staff-mobile/messages' && unreadCount > 0 && (
